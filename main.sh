...
if [ -f "centos-arm64/root/koimux_bot/koishi.yml" ]; then
	#遍历当前目录中的accounts目录
	for dir in $(ls -d centos-arm64/root/koimux_bot/accounts/*/); 
	do
	#获取device.json文件路径
...
wget https://github.com/initialencounter/koimux_bot/releases/download/koimux-%E5%A4%87%E4%BB%BD%E5%8C%85-v2.0/koimux-.-v2.0.tar.gz
cd ~
cd ~
}
function BOOT(){
